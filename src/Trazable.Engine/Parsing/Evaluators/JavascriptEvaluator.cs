    ...
    public class JavascriptEvaluator : Evaluator
    {
        ...
        public dynamic Eval(Token token)
        {
            ...
            switch (token.Name)
            {
                ...
                case "Identifier":
                    dynamic returnvalue = this.environment[token.Text];
                    if (this.environment.ExistsBinding(token.Text))
                    {
        ...
        /// <param name="container">The container.</param>
        /// <returns></returns>
        public static dynamic RunScript(string s, TemplateContainer container = null)
        {
